...
describe('Basic Proxy Operations', () => {
  describe('rules', () => {
    it('should proxy with a exactly matching pathname rule', async () => {
      ...
      const proxy = createProxy([{ pathname: '/abc', dest: s1.url }])
      await listen(proxy)
    ...
    it('should proxy with a wildcard matching pathname rule', async () => {
      const s1 = await createInfoServer()
      const proxy = createProxy([{ pathname: '/abc', dest: 'http://localhost' }, { pathname: '/blog/**', dest: s1.url }])
      await listen(proxy)
    ...
    it('should send 404 if no matching rule found', async () => {
      const s1 = await createInfoServer()
      const proxy = createProxy([{ pathname: '/abc', dest: 'http://localhost' }])
      await listen(proxy)
    ...
    it('should proxy to the first matching rule', async () => {
      const s1 = await createInfoServer()
      const proxy = createProxy([{ pathname: '/abc/**', dest: s1.url }, { pathname: '/abc/blog/**', dest: 'http://localhost' }])
      await listen(proxy)
  ...
  describe('methods', () => {
    it('should proxy for a method in the list', async () => {
      const s1 = await createInfoServer()
      const proxy = createProxy([{ pathname: '/blog/**', method: ['GET', 'POST'], dest: s1.url }])
      await listen(proxy)
    ...
    it('should proxy for any method if no methods provided', async () => {
      const s1 = await createInfoServer()
      const proxy = createProxy([{ pathname: '/blog/**', dest: s1.url }])
      await listen(proxy)
  ...
  describe('other', () => {
    ...
    it('should forward original status code', async () => {
      const s1 = await createInfoServer()
      const proxy = createProxy([{ pathname: '/**', dest: s1.url }])
      await listen(proxy)
    ...
    it('should send back response headers', async () => {
      ...
      const proxy = createProxy([{ pathname: '/blog/**', dest: `http://localhost:${s1.address().port}` }])
      await listen(proxy)
